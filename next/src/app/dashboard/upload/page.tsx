...
export default function UploadPage() {
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDropRejected: (rejections) => {
      ...
      if (err?.code === "file-too-large") setError("File is too large. Maximum size is 500MB.");
      else setError("Only MP3 and MP4 files are supported.");
    },
  });
  ...
  async function handleUpload() {
    ...
    try {
      ...
      setProgress(20);
      const { error: storageError } = await supabase.storage.from("audio-uploads").upload(storagePath, file, { contentType: file.type, upsert: false });
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
        ...
        <CardContent>
          {file ? (
            <div className="flex items-center justify-between p-4 rounded-xl bg-secondary">
              ...
              <button onClick={() => setFile(null)} className="p-1.5 hover:bg-border-hover rounded-lg cursor-pointer transition-colors">
                <X className="h-4 w-4" />
              </button>
            </div>
          ) : (
            <div {...getRootProps()} className={`border-2 border-dashed rounded-2xl p-12 text-center cursor-pointer transition-all duration-200 ${isDragActive ? "border-primary bg-primary-light" : "border-border hover:border-primary/50"}`}>
              <input {...getInputProps()} />
              <Upload className="mx-auto h-12 w-12 text-foreground-muted mb-4" />
              <p className="text-lg font-medium">{isDragActive ? "Drop your file here" : "Drop your MP3 or MP4 file here"}</p>
      ...
      <Card>
        <CardContent>
          <label className="block text-sm font-medium mb-2">Translate transcription to another language? (optional)</label>
          <select value={targetLanguage} onChange={(e) => setTargetLanguage(e.target.value)} className="w-full border border-border rounded-xl p-3 text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all duration-200">
            <option value="">No translation — keep original language</option>
            {TRANSLATION_TARGETS.map((lang) => <option key={lang.code} value={lang.code}>{lang.name}</option>)}
          </select>
        </CardContent>
      </Card>
      {uploading && (
      ...
      <Button className="w-full" size="lg" onClick={handleUpload} disabled={!file || uploading} loading={uploading}>
        Start Transcription
      </Button>
    </div>
  );
