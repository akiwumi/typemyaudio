        ...
        <CardContent className="space-y-4">
            ...
            <div className="text-sm text-foreground-muted">
              {subscription.cancel_at_period_end ? (
                <div className="flex items-center gap-2 text-amber-700 bg-amber-50 rounded-xl p-3">
                  ...
                  <span>
                    Your subscription will end on <strong>{formatDate(subscription.subscription_end)}</strong>
                  </span>
                </div>
              ) : (
          ...
          <div className="flex gap-3">
            ...
            {subscription?.tier !== "free" && !subscription?.cancel_at_period_end && (
              <Button variant="outline" onClick={handleCancel} loading={actionLoading}>Cancel subscription</Button>
            )}
            {subscription?.cancel_at_period_end && (
              <Button onClick={handleReactivate} loading={actionLoading}>Reactivate subscription</Button>
            )}
          </div>
        </CardContent>
      </Card>
