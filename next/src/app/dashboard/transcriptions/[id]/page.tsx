  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        <Link href="/dashboard/transcriptions" className="p-2 hover:bg-secondary rounded-xl cursor-pointer transition-colors">
        ...
        <Badge variant={transcription.status === "completed" ? "success" : transcription.status === "failed" ? "error" : "info"}>
          {transcription.status}
        </Badge>
      </div>
      ...
      {transcription.status === "completed" && (
        <>
          <div className="flex items-center gap-2 flex-wrap">
            {transcription.translated_text && (
              <Button variant={showTranslation ? "primary" : "outline"} size="sm" onClick={() => setShowTranslation(!showTranslation)}>
                <Languages className="h-4 w-4" />
                {showTranslation ? "Show original" : "Show translation"}
              </Button>
            )}
            <Button variant={editing ? "primary" : "outline"} size="sm" onClick={() => { if (editing) handleSave(); else setEditing(true); }} loading={saving}>
            ...
            <div className="flex items-center gap-2">
              ...
              {["txt", "pdf", "docx"].map((fmt) => (
                <Button key={fmt} variant="outline" size="sm" onClick={() => handleExport(fmt)} disabled={exporting}>
                    ...
                    <div key={i} className="flex gap-3 group hover:bg-surface p-2 rounded-lg transition-colors">
                      <button
                        onClick={() => {
                          ...
                          if (player) {
                            (player as HTMLMediaElement).currentTime = s.start;
